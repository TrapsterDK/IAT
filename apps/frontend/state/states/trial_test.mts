...
test("advanceSessionAfterCompletedTrial advances to the next block after the final trial", () => {
  ...
  assert.deepEqual(advanceResult.session.blockUploads.queuedBlockUploads, [
    {
      blockIndex: 1,
      lastError: null,
...
test("advanceSessionAfterCompletedTrial finalizes after the last block completes", () => {
  ...
  assert.equal(advanceResult.session.state, SessionStateKind.Finalizing);
  assert.equal(advanceResult.session.pendingScoreError, null);
  assert.deepEqual(advanceResult.session.blockUploads.queuedBlockUploads, [
    {
      blockIndex: 1,
      lastError: null,
