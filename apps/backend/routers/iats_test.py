...
def test_list_iats_returns_summaries(iat_router_client: TestClient) -> None:
    # Given: one backend app with one configured IAT.

    # When: the client requests the IAT list.
    response = iat_router_client.get("/api/iats")
...
def test_get_iat_returns_detail_with_public_image_url(iat_router_client: TestClient) -> None:
    # Given: one backend app with one configured image-backed IAT.

    # When: the client requests the IAT detail.
    response = iat_router_client.get("/api/iats/sample-iat")
...
def test_get_iat_returns_not_found_for_unknown_slug(iat_router_client: TestClient) -> None:
    # Given: one backend app with no matching IAT slug.

    # When: the client requests one unavailable IAT.
    response = iat_router_client.get("/api/iats/missing-iat")
